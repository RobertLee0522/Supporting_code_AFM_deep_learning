...
- **2026-07-14 — `zerotrain/blind_deconvolution.py`：新增「匯出 Section 結果」（CSV 逐點 + 摘要 txt + PNG）**
  ...
  - **`_vwall_profile`（逐高度掃描填充）→ `_vwall_curve`（逐節點）**：鎖定含游標特徵的峰後，
    ...
    根治掃描填充版在游標點低時被基線連通區污染出假平台的問題。
  - 驗證（30nm.003 長線，游標低 z*≈3.4）：綠折線 34 節點、x[117,139]、峰高=影像 7.44，
    乾淨無方塊；越近基部（Δz 大）收窄越多。

- **2026-07-14 — `zerotrain/blind_deconvolution.py`：修垂直還原綠曲線在剖面兩端冒假方塊**
  - **症狀**：垂直壁綠曲線在遠離特徵處（x=15~40、70~90…）冒出跳到頂端的矩形方塊。
  - **根因**：`_vwall_profile` 逐高度呼叫 `_chord_1d`（含自動凸/凹方向 + 有界優先），
    掃描不同高度時它會**跳去抓基線/雜訊的區段**（非游標所指特徵），累積成假方塊。
  - **修正**：`_vwall_profile` 改為**鎖定含游標特徵的峰**當種子，逐高度只沿該峰**連通往兩側走**
    （簡單 while 擴張 + 亞像素內插），不再用自動方向。綠曲線只落在特徵附近。
  - 驗證（30nm.003 長線）：綠曲線 >base 範圍限縮在 [117,137]nm（峰 130），兩端乾淨無方塊。
...
- **2026-07-14 — `zerotrain/blind_deconvolution.py`：垂直壁模式畫完整綠色還原曲線**
  ...
  - **`_measure`**：影像/還原/垂直壁三種都加「長寬比 (大/小):1」。垂直壁模式對 X、Y 兩軸各減
    ...
    核取切換走 `_on_vwall_toggle`（重量測 ⑤ + 重繪 Section）。
  - 驗證（30nm.003, R8/θ20.7°, 半高）：影像 ⌀22.4 比1.04；一般還原 19.1×17.4 比1.09；
    垂直壁 14.2×13.3 比1.07——三者長寬比皆 ~1，確認圓形。

- **2026-07-14 — `zerotrain/blind_deconvolution.py`：新增「已知垂直側壁」幾何還原模式**
  - **需求**：形態學 erosion 把特徵當圓弧、對垂直壁保守（少修）。若使用者**已知樣品是垂直側壁**，
    可用幾何關係算更大的正確修正：垂直壁單邊撐寬 = 探針在深度 Δz 的側向寬 `w(Δz)`。
  - **`_tip_lateral(Δz)`**：球冠區 `√(2R·Δz−Δz²)`、錐區 `R·sinθ+(Δz−R(1−cosθ))·tanθ`（θ 非對稱取較大軸）。
  - **GUI**：⑤ 新增「☑ 已知垂直側壁」核取；勾選時 Section 的「還原」改為
    **垂直壁真實寬 = 影像寬 − 2·w(Δz)**（Δz＝特徵頂到量測高度），畫綠色真實寬弦、綠色表格列，
    文字顯示「撐寬量 2·w（垂直壁, 深 Δz）」。未勾維持形態學 erosion 還原。
  - 驗證（30nm.003, R8/θ20.7°）：游標半高附近，一般模式 還原10.42/差+4.54；垂直壁模式
    真實寬8.36/撐寬+6.61（Δz=1.8nm）。量越低 Δz 越大、撐寬量越大。

- **2026-07-13 — `zerotrain/blind_deconvolution.py`：Section 寬度自動判斷凸起/凹孔方向（樣品類型選錯也量得對）**
  - **症狀**：樣品類型誤選「孔洞」量凸起時，左緣跑到 0、還原寬度 > 影像寬度、寬度差變負
    （−2.05nm）——因孔洞邏輯 `z ≤ zlev` 量到的是**基線**（貼到剖面邊界）而非那顆凸起。
  - **修正**：`_level_cross` 同時算 `z ≥ zlev`（凸起候選）與 `z ≤ zlev`（凹孔候選）兩連通區，
    **『有界的特徵』優先於『貼到陣列邊界的基線』**（`bounded()` 判斷兩緣是否為真交點）；
    樣品類型只當偏好順序。即使 radio 選錯，Section 仍量到游標所指的真實特徵。
  - 驗證（30nm.003 凸起）：樣品選「孔洞」→ 影像 21.13/還原 17.47/差 +3.66；選「凸起」→
    22.19/18.60/+3.59，兩者一致、還原恆 ≤ 影像（正撐寬量）。

- **2026-07-13 — `zerotrain/blind_deconvolution.py`：修 Section 寬度量到整條線的離譜值（改量游標所指特徵）**
  - **症狀**：長剖面線時 Section 量出影像寬 80.24 / 還原 45.78nm、寬度差 +34nm，橘弦在左半、
    紅弦在右半完全對不起來。
  - **根因**：`_level_cross` 從**全域最高峰**往兩側找 z\* 交點，與游標所指特徵脫鉤。當基線有
    傾斜/掃描條紋雜訊、或線掃到旁邊特徵時，「≥z\*」被切成十幾段，walk 一路走到陣列邊界 →
    左緣=0、右緣=線長，量出橫跨整條線的假寬度。
  - **修正**：`_level_cross(surf, zlev, xpos)` 改用 `scipy.ndimage.label` 標記所有連通區，
    取**含游標 xpos（或最靠近）的那一區**量左右緣，而非全域峰；`xpos=None` 退回全域（供預設）。
    `_curve_chord` 傳入 xpos。
  - 驗證（30nm.003 長線）：游標 135nm→影像 12.29/還原 7.46/差 +4.83；游標 150→20.5/16.7/+3.84；
    四交點緊貼游標所指凸起、基線雜訊不再污染、還原恆 ≤ 影像。
...
- **2026-07-10 — `blind_deconvolution.py`：Section 改混合式量測高度 z*（以影像高度為基準）**
  - **需求**：B 型（各曲線自己高度）比的是兩個不同高度截面，側壁斜率會污染寬度差、還原失真。
    改混合式——**點游標取『影像曲線』在該點的高度 z\*，影像與還原都用同一個 z\* 量左右緣**，
    四點同高、X 相減乾淨＝純探針撐寬量。物理依據：erosion 不改高度基準，同高度下
    `recon ≤ image` 恆成立 → 寬度差 ≥ 0（乾淨定義）。
  - **實作**：新增 `_measure_level(xpos)`（影像在 xpos 的內插高度）；`_curve_chord` 改為對任意
    surface 都以此 z\* 呼叫 `_level_cross`。文字改「量測高度 z\*(影像基準)/影像寬度/還原寬度/
    寬度差」，表格兩列量測高度欄相同。
  - 驗證（30nm.003）：游標 28→高 5.71 影像15.14/還原11.85 差+3.29；22→高2.93 25.70/21.36 +4.34；
    15→高−0.42 45.94/39.96 +5.98。兩列高度必相同、還原恆 ≤ 影像（正的撐寬量，越近底越大）。
    對比舊 B 型 28nm 得污染值 +0.95，混合式為乾淨 +3.29。

- **2026-07-10 — `blind_deconvolution.py`：Section 等高量測改 B 型——各曲線在游標處「自己的高度」取水平弦**
  - **需求確認（AskUserQuestion）**：使用者要的不是四點共用一條高度線（A 型），而是
    **各曲線各自等高**（B 型）——兩橘點同高（影像曲線自己的高度）、兩紅點同高
    （還原曲線自己的高度），兩弦高度可不同，但**同曲線的左右點絕不一高一低**。
  - **控制改為位置游標**：`section['zlev']` → `section['xpos']`（灰垂線，剖面上點/拖移動）。
    `_curve_chord(surf, xpos)`：取該曲線在 xpos 的高度（`np.interp`）→ `_level_cross`
    找同高左右緣 → 水平弦。預設 xpos＝影像半高左緣（`_default_xpos`，落在上升坡）。
  - **顯示**：橘弦（兩橘點+橘虛線，必同高）、紅弦（兩紅點+紅虛線，必同高）；文字四行
    「游標位置 / 影像 寬×高(橘) / 還原 寬×高(紅) / 寬度差(綠)」；表格每列各自的量測高度。
  - 驗證（30nm.003）：游標 28nm → 影像弦高 5.71 寬 15.14、還原弦高 5.06 寬 14.19、
    差 +0.95nm；左緣皆=游標位置 28.00 ✓、各弦兩端同高 ✓。

- **2026-07-10 — `blind_deconvolution.py`：Section 改「等高量測」——一條高度線、兩曲線同高取交點**
  - **需求**：雙游標各自戳在不同高度「看得好痛苦」；要「橘點在 10、紅線的點也在 10，
    兩者 X 相減就知道距離」——即**同一高度**下比較影像/還原寬度。
  - **量測模型重構**：`section` 的 `c1/c2` 游標 → 單一 **`zlev` 量測高度**；剖面上
    **點/拖 = 移動灰色水平高度線**。`_level_cross(surf, zlev)`：從峰頂(凸起)/谷底(孔洞)
    向兩側走到跨越 zlev 處（亞像素內插）→ 影像與還原各得同高左右交點。
    建線預設 zlev=半高（`_default_level`）。
  - **顯示**：灰虛線=量測高度；橘點×2=影像交點、紅點×2=還原交點，**全部落在同一條線上**，
    橘在外紅在內一眼看出撐寬；文字四行色碼——量測高度/影像寬度(橘)/還原寬度(紅)/
    **寬度差(綠)=探針撐寬量**。交點同步映射回掃描影像白線上（橘/紅小點）。
    表格改「量測高度/左緣/右緣/寬度」。刪除 `_pair_stats/_event_to_t/_section_readout`。
  - 驗證（30nm.003）：半高 3.43nm→影像 24.24/還原 19.80；拖到 z=0.75→影像 32.76
    （≈操作人底寬 30nm）/還原 28.30/寬度差 +4.45nm。

- **2026-07-10 — `blind_deconvolution.py`：ΔZ 虛線改 L 形接準兩點 + 全形小數點容錯（真因）**
  - **ΔZ 虛線「對不起來」**：原本水平虛線固定畫在點1高度、點2 不在該高度時線就懸空。
    改為 **L 形折線**（水平段=跨距、垂直段=降/升到點2），兩端準確落在影像(橘)/還原(紅)
    各自的兩個交點圓點上；拖曳同步更新。
  - **「無法輸入小數點」真因**：θ 欄位一直支援半形小數（`float`），但**中文輸入法打出的是
    全形「。／．／，」**，`float()` 解析失敗 → 自動重跑靜默略過，看起來像「不能輸入小數點」。
    新增模組級 `_fnum()`：正規化全形句號/逗號為半形句點再解析；GUI 全部 16 處數值欄
    （R/θ/θx/θy/px_nm/視窗半徑/半深比例）改用 `_fnum`。驗證 `'20。7'→20.7` 端到端跑通。
...
- **2026-07-09 — `blind_deconvolution.py`：NanoScope Section 式互動剖面線（拖線/拖游標即時量測）**
  ...
  - **互動設計**（`_on_press/_on_motion/_on_release` 三段滑鼠事件取代單一 click）：
    影像上**按住拖曳**=拉剖面線（放開不動仍=原「點特徵」，以移動 >4 螢幕px 區分）；
    線的**端點（白方塊）可拖曳**調整；Section 面板上**點/拖=移動較近的游標**（NanoScope 手感）。
    「✕ 剖面線」鈕清除。剖面線同步畫在輸入影像與還原圖（白線+青/洋紅游標點）。
  - **Section 面板**（取代右上「特徵剖面 X」，清除後還原）：沿線取樣
    （`map_coordinates` 雙線性，240 點）畫影像(橘)/還原(紅)曲線 + 兩條游標虛線，
    標題即時顯示**游標間距離(nm) 與 Δz**（影像/還原各自）。
  - **拖曳效能**：拖曳中走 `_update_section_artists()` 只更新 artists 資料 +
    `draw_idle()`（不整張 clf 重繪），放開才完整 `_refresh()`——保持滑順。
  - 驗證（30nm.003）：拖線 (100,118)→(160,122) 跨中央凸起、游標拉到兩腳讀「距離 31.5nm」
    ≈ 操作人底寬 30nm ✓；Δz影像/還原即時更新；點特徵/門檻點選功能不受影響。
    換載新影像自動清線。「↔」缺字型改用「距離」文字。

- **2026-07-09 — `blind_deconvolution.py`：修正「重新輸入數值後結果不更新」——參數變更自動重跑**
  - **問題**：改 R/θ 等數值後需手動重按 ▶ 才更新（使用者以為壞掉）；且**切換樣品類型
    完全沒反應**（.000 需以新基線方向重新載入去傾斜，舊版沒做）——此為真 bug。
  - **自動重跑機制**：R/θ/θx/θy/px_nm/視窗半徑任一變更 → 防抖 700ms 後自動重建探針
    並重跑去卷積（`_schedule_auto_run`/`_auto_run`；`_busy` 旗標避免並行、執行中改值
    300ms 後重試；輸入中的不完整數值靜默略過不跳錯）。載入影像後也自動跑第一輪。
    半深比例變更 → 防抖 500ms 自動重量測（`_schedule_measure`）。
  - **樣品類型切換**（`_on_sample_change`）：Nanoscope 檔以新基線方向（孔洞 pct95/凸起
    pct10）**重新載入**、清空舊結果、重算視窗半徑並自動重跑。
  - 驗證（30nm.003）：載入自動跑 ✓；改 θ=35 → 視窗 5→8、結果自動更新 ✓；切孔洞 →
    影像重載+自動重跑 ✓；改半深 0.1 → 自動重量測 ✓。

- **2026-07-09 — `blind_deconvolution.py`：剖面圖點選高度＝設定量測門檻（操作人互動量法）**
  - **需求**：使用者要能在剖面圖上「點自己要量的位置」計算大小——即操作人動作
    「從平緩線開始上升處」點下去，就以該高度量寬。
  - **`_on_click` 新增剖面圖分支**：點「特徵剖面 X/Y」任一格時，取點選的 **Z 高度**，
    以影像基線（凸起 pct10 / 孔洞 pct95）與主特徵振幅換算成門檻比例
    `frac=(z−base)/amp`（夾在 0.05–0.95），自動填入「半深比例」欄並重新量測——
    2D 標註、剖面虛線、面板數值全部同步跳到該高度。點基線附近＝底部全寬（操作人量法）、
    點半高＝FWHM。`_refresh` 存 `_ax_profx/_ax_profy` 供分支判定。
  - **驗證**（30nm.003, bump, R=8/θ=20.7）：點 z=0.75nm→比例 0.10、影像 28.9/還原 ⌀25.9；
    點 z=3.7nm→0.50 回 FWHM；點 z=20nm→安全夾 0.95。截圖確認虛線與數值落在點選高度。

- **2026-07-09 — `blind_deconvolution.py`：GUI 新增「特徵剖面圖」X/Y 兩格（對齊操作人 2D 剖面量法）**
  - **背景**：操作人以 2D 剖面「從平緩線上升處量到下降回平面」得 ~30nm，與程式 FWHM@50%
    的 22.4/18.3nm 差距大——量測基準不同（底部全寬 vs 半高全寬）。實測對照：門檻 10% 時
    影像 31.9×27.8nm ≈ 操作人 30nm ✓（門檻勿低於 ~10%，背景雜訊 std≈0.24nm 會亂抓）。
  - **GUI 圖區 2×3 → 2×4**：新增「特徵剖面 X / Y」兩格——沿量測線畫高度曲線
    （影像橘、還原紅），量測跨距以虛線畫在**各自門檻高度**、標註寬度數值（影像標線上、
    還原標線下防重疊）；視野鎖定特徵 ±2.5× 跨距。與操作人在同種圖上核對量的位置與基準。
  - `_draw_feature_profile(ax, horiz)`；量測後自動更新，未量測顯示佔位標題。
    驗證：30nm.003 @10% 剖面圖 X「影像 31.9 / 還原 26.7nm」與面板數值一致。
...
- **2026-07-07 — `blind_deconvolution.py`：還原表面特徵寬度量測（FWHM，X/Y + 等效直徑）**
  ...
  - **新增 `measure_feature_width(surface, px_nm, sample, frac=0.5)`**：把特徵轉成正向振幅
    （孔洞=深度、凸起=高度）→ 取『含最深/最高點的連通區』為主特徵 → 量測**經其形心**的
    X、Y 剖面在 `frac×振幅` 門檻的連續跨距（FWHM，線性內插到亞像素）→ 另以門檻連通區面積
    算等效直徑 ⌀=2√(A/π)。**用形心而非 argmax**：平底特徵的 argmax 會落在邊角、剖面量到短弦
    而失真（單元測試抓到並修正）。`frac` 預設 0.5（半深/半高全寬）。
  - **GUI**：⑤ 結果新增「影像寬度 ⌀ / 還原寬度(X×Y ⌀)」兩列、「半深比例」輸入 + 「📏 量測寬度」鈕；
    去卷積完成後**自動量測**。**輸入影像面板（橘色）與還原面板（紅色）各自疊上** X/Y 量測線 +
    形心十字 + 數值標註（`_draw_width(ax, meas, color)`），一眼對照去卷積收窄多少；save_panels
    存圖亦同步（`meas`/`meas_in` 兩參數）。**CLI**：去卷積後印出「還原寬度@50% X×Y (⌀) ← 影像 ⌀」對照。
  - **驗證**：合成圓孔(⌀40)/橢圓(60×28)/凸起(⌀32) 量測值皆吻合（±1px 硬邊內插誤差）；
    真實 `30nm.003`（bump, R=8/θ=20.7）還原 ⌀18.4nm ← 影像 ⌀22.4nm（erosion 去除 tip broadening）；
    GUI 測試確認自動量測、標籤更新、改半深比例重量測、量測線繪製皆正確。圖中 ⌀ 改 ASCII `D=` 避免缺字。
